...
function SearchBar({ onSearch }: { onSearch: (form: SearchForm) => void }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-[8px] border border-hairline bg-background p-4 shadow-[var(--shadow-subtle)] md:p-5">
      ...
      <p className="mt-3 text-[14px] leading-6 text-ink-muted">Use dates to check Swank availability. Browse below without dates using our editorial cards and affiliate links.</p>
    </form>
  );
}
...
async function fetchAvailability(hotels: Hotel[], form: SearchForm): Promise<Record<string, Availability>> {
  ...
  return Object.fromEntries(
    hotels.map((hotel) => [hotel.id, available[hotel.id] ?? { status: "unavailable" }]),
  );
}
    ...
    <article className="flex flex-col overflow-hidden rounded-[8px] border border-hairline bg-background transition-shadow duration-[180ms] hover:shadow-[var(--shadow-subtle)]">
      ...
      <div className="flex flex-1 flex-col p-5 md:p-6">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <h3 className="text-[24px] leading-8 font-normal text-ink">{hotel.name}</h3>
          <span className="badge-pill bg-soft uppercase tracking-wide text-ink-muted">{hotel.tier}</span>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {hotel.affiliateLinks.map((link) => (
              <a key={link.label} href={link.href} className="btn-base button-secondary w-full" target="_blank" rel="noopener noreferrer">{link.label}</a>
            ))}
            {isAvailable && (
              <a href={availability.bookingUrl ?? "#"} className="btn-base button-primary w-full justify-between">
                <span>Book with Swank</span>
                {availability.price && <span className="badge-pill shrink-0 whitespace-nowrap bg-ink uppercase tracking-wide text-white">{availability.price}</span>}
              </a>
            )}
          </div>
          ...
          <button onClick={() => setOpen((value) => !value)} className="text-link self-start text-[15px] leading-6">Why this value? {open ? "-" : "+"}</button>
    ...
    <footer className="border-t border-hairline bg-background py-10 md:py-16">
      <div className="page-container">
        <div className="grid gap-10 md:grid-cols-3">
          <div><p className="mb-4 text-[12px] font-medium uppercase tracking-[0.14em] text-ink-muted">Services</p><div className="space-y-3 text-[15px] leading-6 text-ink-muted"><p>See how we curate our picks</p><p><Link to="/book-direct" className="text-link">Book a hotel stay</Link></p><p>5990 more hotels with perks</p><p>We VIP you with Fora</p></div></div>
          <div className="flex items-start gap-4"><div className="rounded-[4px] border border-line px-4 py-3 text-center text-[12px] font-medium uppercase leading-[18px] tracking-wide text-ink-muted">IATA<br />ACCREDITED</div><div className="rounded-[4px] border border-line px-4 py-3 text-center text-[12px] font-medium uppercase leading-[18px] tracking-wide text-ink-muted">FORA<br />CERTIFIED</div></div>
          <div><p className="mb-4 text-[12px] font-medium uppercase tracking-[0.14em] text-ink-muted">Community</p><div className="space-y-3 text-[15px] leading-6 text-ink-muted"><p><Link to="/concierge" className="text-link">Why we're different</Link></p><p>Blog</p><p>FAQ</p><p>Newsletter</p></div></div>
        </div>
        <div className="mt-12 flex flex-col gap-3 border-t border-hairline pt-6 text-[14px] text-ink-muted sm:flex-row sm:items-center sm:justify-between"><span>© 2026, Swank Guide</span><div className="flex flex-wrap gap-6"><span>Contact</span><span>Terms of Use</span><span>Privacy</span></div></div>
      </div>
    </footer>
  );
}
...
export default function BookDirectHybrid() {
  ...
  const visibleHotels = useMemo(() => {
    ...
    return HOTELS.filter((hotel) => [hotel.name, hotel.destination, hotel.tier].some((value) => value.toLowerCase().includes(query)));
  }, [search]);
  ...
  async function handleSearch(form: SearchForm) {
    ...
    try {
      ...
      setError("We couldn't check Swank availability for those dates. You can still browse below or use the affiliate links.");
      setAvailability({});
    }
  }
    ...
    <div className="flex min-h-screen flex-col bg-background text-ink antialiased">
      ...
      <main className="flex-1">
        <section className="book-direct-hero">
          ...
          <div className="page-container book-direct-hero__content"><Eyebrow className="text-white/80">Book Direct</Eyebrow><h1 className="display-heading mt-3 max-w-[760px] text-white">Search directly or browse our picks.</h1></div>
        </section>
        ...
        <section className="page-container -mt-10 relative z-10 pb-16 md:pb-24">
          ...
          <section className="mt-12 border-b border-hairline pb-12" aria-labelledby="most-booked-title">
            <div className="mb-8 max-w-[720px]"><Eyebrow className="mb-3 text-ink-muted">Most booked hotels</Eyebrow><h2 id="most-booked-title" className="section-heading text-ink">Start with our guest favorites.</h2><p className="mt-3 text-[16px] leading-7 text-ink-muted">Browse without dates, then use the search bar when you are ready to check live Swank availability.</p></div>
            <div className="listing-grid">{MOST_BOOKED_HOTELS.map((hotel) => <article key={hotel} className="group flex flex-col bg-background transition-all duration-[180ms] hover:-translate-y-0.5"><div className="hotel-card__media"><span className="max-w-[82%] px-4 text-center text-[12px] leading-[18px] text-ink-muted">[ {hotel} ]</span><span className="badge-pill absolute bottom-3 left-3 bg-brand uppercase tracking-wide text-ink">Most booked</span></div><div className="listing-card__body"><h3 className="listing-card__name">{hotel}</h3></div></article>)}</div>
          </section>

          <section className="mt-12 border-b border-hairline pb-12" aria-labelledby="destination-nav-title">
            <div className="mb-6 max-w-[720px]"><Eyebrow className="mb-3 text-ink-muted">Browse by destination</Eyebrow><h2 id="destination-nav-title" className="section-heading text-ink">Navigate without entering dates.</h2></div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{DESTINATIONS.map((destination) => destination === "Bangkok" ? <Link key={destination} to="/book-direct/bangkok" className="flex min-h-14 items-center justify-between rounded-[8px] border border-line bg-background px-4 py-3 text-[16px] text-ink hover:bg-soft"><span>{destination}</span><span aria-hidden="true">›</span></Link> : <div key={destination} className="flex min-h-14 items-center justify-between rounded-[8px] border border-hairline bg-soft px-4 py-3 text-[16px] text-ink-muted"><span>{destination}</span><span className="badge-pill bg-background uppercase tracking-wide text-ink-muted">Coming soon</span></div>)}</div>
          </section>

          <div className="mt-12 max-w-[820px]"><p className="text-[16px] leading-7 text-ink-muted"><strong className="font-medium text-ink">No sponsorships. No paid placements. Every recommendation is independent.</strong>{" "}The cards below use Swank photos and descriptions. WebBeds only adds live price, availability, and the Book with Swank path after a date search.</p></div>

          {tiers.map((tier) => {
            const hotels = visibleHotels.filter((hotel) => hotel.tier === tier);
            if (hotels.length === 0) return null;
            return <section key={tier} className="mt-12 md:mt-16"><div className="mb-8 flex flex-wrap items-baseline gap-x-4 gap-y-1 border-b border-hairline pb-4"><h2 className="text-[22px] leading-[30px] font-normal text-ink">{tier}</h2><span className="text-[15px] leading-6 text-ink-muted">{TIER_SUBTITLES[tier]}</span></div><div className="grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-2 md:gap-y-12 xl:grid-cols-3">{hotels.map((hotel) => <HotelCard key={hotel.id} hotel={hotel} availability={availability[hotel.id]} />)}</div></section>;
          })}

          {visibleHotels.length === 0 && <div className="mt-12 rounded-[8px] border border-hairline bg-soft p-6 md:p-8"><p className="text-[16px] leading-7 text-ink-muted">We do not have a Swank Discovery card for that destination yet. Try Bangkok, or use Concierge if you want our team to help plan it.</p><Link to="/concierge" className="btn-base button-accent mt-5">Switch to Concierge</Link></div>}

          <div className="mt-12 rounded-[8px] border border-hairline bg-soft p-6 md:p-8"><p className="text-[16px] leading-7 text-ink-muted"><strong className="font-medium text-ink">Book through any of our links and you'll receive our trip-prep kit plus our destination e-book</strong>: where to eat, what to skip, and the spots worth your time. Forward your confirmation to <span className="text-link">vip@swankguide.com</span></p></div>
        </section>
      </main>
      <Footer />
    </div>
