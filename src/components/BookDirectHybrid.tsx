...
const HOTELS: Hotel[] = [
  {
    ...
    affiliateLinks: [{ label: "Affiliate link", href: "#" }],
  },
  {
    id: "hotel-standard",
...
const MOCK_AVAILABLE_HOTELS: Record<string, string> = {
  "hotel-como": "from $185/night",
  "hotel-peninsula": "from $310/night",
  "hotel-fourseasons": "from $640/night",
};
  ...
  return (
    <form onSubmit={submit} className="rounded-[8px] border border-hairline bg-background p-4 shadow-[var(--shadow-subtle)] md:p-5">
      <div className="grid gap-4 md:grid-cols-[minmax(220px,2fr)_minmax(150px,1fr)_minmax(150px,1fr)_minmax(110px,0.7fr)_auto] md:items-end">
        ...
        <button type="submit" className="btn-base button-accent min-h-14 whitespace-nowrap">
          Search hotels
        </button>
      </div>
      <p className="mt-3 text-[14px] leading-6 text-ink-muted">
        Search updates availability without removing the Swank cards or affiliate links.
      </p>
    </form>
  );
}
...
async function fetchAvailability(hotels: Hotel[], form: SearchForm): Promise<Record<string, Availability>> {
  ...
  if (!WEBBEDS_ENDPOINT) {
    return Object.fromEntries(
      hotels.map((hotel) => [
        hotel.id,
        MOCK_AVAILABLE_HOTELS[hotel.id]
          ? { status: "available", price: MOCK_AVAILABLE_HOTELS[hotel.id] }
          : { status: "unavailable" },
      ]),
    );
  }
  ...
  if (!response.ok) {
    throw new Error("WebBeds availability request failed");
  }

  return (await response.json()) as Record<string, Availability>;
}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            {isAvailable && (
              <button className="btn-base button-primary w-full justify-between">
                ...
                <span className="badge-pill shrink-0 whitespace-nowrap bg-ink uppercase tracking-wide text-white">{availability.price}</span>
              </button>
            )}
          </div>

          {isLoading && (
            <div className="rounded-[8px] border border-hairline bg-soft p-4 text-[15px] leading-6 text-ink-muted">
              Checking Swank availability for your dates...
            </div>
          )}

          <button onClick={() => setOpen((value) => !value)} className="text-link self-start text-[15px] leading-6">
            {open ? "Why this value?" : "Why this value?"} {open ? "-" : "+"}
          </button>
          {open && (
...
export default function BookDirectHybrid() {
  ...
  const visibleHotels = useMemo(() => {
    ...
    return HOTELS.filter((hotel) =>
      [hotel.name, hotel.destination, hotel.tier].some((value) => value.toLowerCase().includes(query)),
    );
  }, [search]);
  ...
  async function handleSearch(form: SearchForm) {
    ...
    const scopedHotels = HOTELS.filter((hotel) => {
      ...
      return !query || [hotel.name, hotel.destination, hotel.tier].some((value) => value.toLowerCase().includes(query));
    });

    setAvailability(
      Object.fromEntries(scopedHotels.map((hotel) => [hotel.id, { status: "loading" as const }])),
    );

    try {
      const result = await fetchAvailability(scopedHotels, form);
      setAvailability(result);
    } catch (err) {
      console.error("[Book Direct] availability failed", err);
      setError("We couldn't check Swank availability for those dates. You can still use the affiliate links below.");
      setAvailability({});
    }
  }
        ...
        <section className="page-container -mt-10 relative z-10 pb-16 md:pb-24">
          ...
          <div className="mt-12 max-w-[820px]">
            <p className="text-[16px] leading-7 text-ink-muted">
              <strong className="font-medium text-ink">No sponsorships. No paid placements. Every recommendation is independent.</strong>{" "}
              Browse the Swank Discovery cards below, or search with dates to add live Swank availability when the hotel is in our WebBeds inventory.
            </p>
            <div className="mt-4 flex flex-wrap gap-x-8 gap-y-2 text-[15px] leading-6 text-ink-muted">
              <p><span className="font-medium text-ink">Swank Tested</span> Don stayed here personally, on his own dime.</p>
              <p><span className="font-medium text-ink">Trusted Pick</span> Visited, vetted, and recommended by our team.</p>
            </div>
          </div>
          ...
          {tiers.map((tier) => {
            ...
            return (
              <section key={tier} className="mt-12 md:mt-16">
                <div className="mb-8 flex flex-wrap items-baseline gap-x-4 gap-y-1 border-b border-hairline pb-4">
                  ...
                  <span className="text-[15px] leading-6 text-ink-muted">{TIER_SUBTITLES[tier]}</span>
                </div>
                <div className="grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-2 md:gap-y-12 xl:grid-cols-3">
                  {hotels.map((hotel) => (
                    <HotelCard key={hotel.id} hotel={hotel} availability={availability[hotel.id]} />
                  ))}
                </div>
              </section>
            );
          ...
          {visibleHotels.length === 0 && (
            <div className="mt-12 rounded-[8px] border border-hairline bg-soft p-6 md:p-8">
              <p className="text-[16px] leading-7 text-ink-muted">
                We do not have a Swank Discovery card for that destination yet. Try Bangkok, or use Concierge if you want our team to help plan it.
              </p>
              <Link to="/concierge" className="btn-base button-accent mt-5">Switch to Concierge</Link>
            </div>
          )}

          <div className="mt-12 rounded-[8px] border border-hairline bg-soft p-6 md:p-8">
            <p className="text-[16px] leading-7 text-ink-muted">
              <strong className="font-medium text-ink">Book through any of our links and you'll receive our trip-prep kit plus our destination e-book</strong>: where to eat, what to skip, and the spots worth your time. Forward your confirmation to <span className="text-link">vip@swankguide.com</span>
            </p>
          </div>
        </section>
      </main>
